...
class Simulate:
    ...
    @jit(nopython=True)
    def Body_Advance_step_jit(self,old_masses_arr,springs_arr, 
                              T,  dt = 0.00007,
                              G = np.array([0, 0, -9.81]),
                              b  = 0.999, Kc = 100000,
                              mu_s = 0.89, mu_k = 0.70, omega = 2*np.pi, 
    ...
    def run_simulation(self, Plot = False, Actuator_on = False, Verbose = False, max_T = 1): 
        ...
        if Plot:
            ...
            while True:
                ...
                masses_arr, _ = self.Body_Advance_step_jit(masses_arr, springs_arr,
                                                           T = self.T, dt = self.dt)
                self.T+=self.dt
                self.global_step +=1
...
if __name__ == "__main__":
    ...
    profiler.enable()
    fitness = sim1.run_simulation(Plot = False, Verbose = True, max_T = 20)
